...
def load_vector_database(_embedding_func, _docs):
    ...
    try: client.delete_collection("cnn_docs")
    except: pass
    return Chroma.from_documents(documents=split_docs, embedding=_embedding_func, collection_name="cnn_docs", client=client)
...
# --- LLM Helpers ---
def get_openai():
    try: return ChatOpenAI(model="gpt-4o-mini", api_key=st.secrets.get("open_ai_api_key", ""), temperature=0)
    except: return None

def get_groq_fallback():
    try: return ChatGroq(model_name="llama-3.3-70b-versatile", groq_api_key=st.secrets["groq_api_key"], temperature=0.7)
    except: return None
...
def get_article_topic(doc):
    ...
    try: return llm.invoke(prompt).content.strip().replace(".", "")
    except: return "Article Content"

def run_hybrid_summarization(relevant_docs):
    groq = get_groq_fallback()
    open_ai = get_openai()
    for model in [open_ai, groq]:
        if model:
            try:
                chain = load_summarize_chain(model, chain_type="stuff")
                res = chain.invoke({"input_documents": relevant_docs})
                return res['output_text'], model.__class__.__name__
            except: continue
    ...
    with st.spinner(f"Analyzing articles for '{query}'..."):
        ...
        if not relevant_docs:
            ...
            def process_doc(doc):
                ...
                return topic, summary_text, model_name, doc.metadata.get('source', 'CNN Lite')
